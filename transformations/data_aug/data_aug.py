...
def checkInputs(img_group):
    img_group_pil = []
    for i in range(len(img_group)):
        # print(type(img_group[i]))
        img_tmp = img_group[i]
        if not isinstance(img_group[i], np.ndarray):
            # print('converting to NUMPY')
            img_tmp = pil2numpy(img_tmp)
        img_group_pil.append(img_tmp)
    return img_group_pil
...
class ClipRandomHorizontalFlip(object):
    def __init__(self):
        pass
        # self.p = p
        # self.img_transform = RandomHorizontalFlip(p)
...
class ClipRandomScale(object):
    def __init__(self, scale = 0.2, diff = False):
        # self.img_transform = RandomScale(scale, diff)
        self.scale = scale
    ...
    def __call__(self, img_group, bboxes):
        ...
        for i, img in enumerate(img_group):
            # img_t, bboxes_t = self.img_transform(img, bboxes[i])
            img_t, bboxes_t = self.imgTransformation(img, bboxes[i])
            imgs.append(img_t)
            bboxes_transformed.append(bboxes_t)
        return imgs, bboxes_transformed

class ClipRandomRotate(object):
    def __init__(self, angle):
        # self.img_transform = RandomRotate(angle)
        self.angle = angle
        ...
        self.random_angle = random.uniform(*self.angle)
    ...
    def imgTransformation(self, img, bboxes):
        ...
        img = rotate_im(img, self.random_angle)
    
        corners = get_corners(bboxes)
    
        corners = np.hstack((corners, bboxes[:,4:]))
    
        corners[:,:8] = rotate_box(corners[:,:8], self.random_angle, cx, cy, h, w)
...
class ClipRandomTranslate(object):
    def __init__(self, translate = 0.2, diff = False):
        ...
        if not self.diff:
            self.translate_factor_y = self.translate_factor_x
    ...
    def imgTransformation(self, img, bboxes):
        ...
        corner_x = int(self.translate_factor_x*img.shape[1])
        corner_y = int(self.translate_factor_y*img.shape[0])
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, img, bboxes):
        img_center = np.array(img.shape[:2])[::-1]/2
        # print('img.shape[:2]: ', img.shape[:2], img_center)
